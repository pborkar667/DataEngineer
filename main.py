...
def produce_data_to_kafka(producer, topic, data):
    try:
        ...
        print("Local producer queue is full, waiting for messages to be delivered.")
        producer.poll(1)  # Wait a bit for the queue to clear

def simulate_journey(producer, device_id):
    while True:
        vehicle_data = generate_vehicle_data(device_id)
        gps_data = generate_gps_data(device_id, vehicle_data['timestamp'])
        traffic_camera_data = generate_traffic_camera_data(device_id, vehicle_data['timestamp'], vehicle_data['location'], 'Nikon-123')
        weather_data = generate_weather_data(device_id, vehicle_data['timestamp'], vehicle_data['location'])
        emergency_incident_data = generate_emergency_incident_data(device_id, vehicle_data['timestamp'], vehicle_data['location'])

        if (
            vehicle_data['location'][0] >= BIRMINGHAM_COORDINATES['latitude'] and
        ...
        # Poll producer events to handle delivery reports and clear the queue
        producer.poll(0)
        producer.flush()

        # Simulate journey time delay
        time.sleep(random.uniform(0.5, 2))  # Adjust timing as needed
